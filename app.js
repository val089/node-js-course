...
// Relations
// sequelize associations - one to many relationship between user and product
Product.belongsTo(User, { constraints: true, onDelete: 'CASCADE' });
User.hasMany(Product); // optional
User.hasOne(Cart);
Cart.belongsTo(User);
// thanks to through return data will be looks like CartItem
...
sequelize
  ...
  .then((user) => {
    if (!user) {
      return User.create({ name: 'Kamil', email: 'kamil@wp.pl' });
    }
    return user;
  })
  .then((user) => {
